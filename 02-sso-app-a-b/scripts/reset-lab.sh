#!/usr/bin/env bash
# 02 ラボを最初からやり直す（App A/B 削除 + ローカル .env 削除）
#
# ホスト（ラボのルート）から:
...
#   --no-recreate 終了後に web / web-b を recreate しない（ホスト実行時のみ）
#
# 環境変数:
#   APP_A_NAME  既定: my-next-02-a
#   APP_B_NAME  既定: my-next-02-b

set -euo pipefail

APP_A_NAME="${APP_A_NAME:-my-next-02-a}"
...
if ! in_lab; then
  ...
  if [[ $status -eq 0 && $NO_RECREATE -eq 0 ]]; then
    echo
    echo "== host: web / web-b を作り直す =="
    docker compose up -d --force-recreate web web-b
    echo "Open http://localhost:3000  /  http://localhost:3001"
    echo "Next: lab -> tenants use -> qs setup (A) -> setup-app-b.sh"
  fi
  exit $status
fi

ENV_A="/workspace/web/.env"
ENV_A_LOCAL="/workspace/web/.env.local"
ENV_B="/workspace/web-b/.env"
ENV_B_LOCAL="/workspace/web-b/.env.local"
AUTH0_CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/auth0"

echo "== reset-lab (A=${APP_A_NAME} / B=${APP_B_NAME}) =="
echo "tenant: $(auth0 tenants list 2>/dev/null | head -n 20 || true)"

if [[ $FORCE -eq 0 ]]; then
  echo
  echo "次を実行します:"
  [[ $KEEP_APP -eq 0 ]] && echo "  - Auth0 App「${APP_A_NAME}」「${APP_B_NAME}」（および .env の Client ID）を削除"
  echo "  - ${ENV_A} / ${ENV_B}（と .env.local）を削除"
...
if [[ $KEEP_APP -eq 0 ]]; then
  ...
  mapfile -t CLIENT_IDS < <(
    { find_client_ids_for "$APP_A_NAME" "$ENV_A"; find_client_ids_for "$APP_B_NAME" "$ENV_B"; } \
      | awk 'NF && !seen[$0]++'
  )
  if [[ ${#CLIENT_IDS[@]} -eq 0 ]]; then
    echo "削除対象の Client ID が見つかりません。スキップします。"
  else
...
echo
echo "== ローカル .env を削除 =="
rm -f "$ENV_A" "$ENV_A_LOCAL" "$ENV_B" "$ENV_B_LOCAL"
echo "removed: web/.env web-b/.env（存在すれば）"
...
echo "  auth0 qs setup --app --type regular --framework nextjs --name ${APP_A_NAME} --port 3000 --no-input"
echo "  bash /workspace/scripts/normalize-env.sh"
echo "  bash /workspace/scripts/setup-app-b.sh"
echo "  exit"
echo
echo "  # ホスト:"
echo "  docker compose up -d --force-recreate web web-b"
echo "  # → :3000 / :3001"
